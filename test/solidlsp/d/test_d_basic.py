...
@pytest.mark.d
class TestDLanguageServer:
    @pytest.mark.parametrize("language_server", [Language.D], indirect=True)
    ...
    def test_find_referencing_symbols(self, language_server: SolidLanguageServer) -> None:
        ...
        for sym in symbols[0]:
            if sym.get("name") == "Helper":
                helper_symbol = sym
                break
        assert helper_symbol is not None, "Could not find 'Helper' function symbol in main.d"
        sel_start = helper_symbol["selectionRange"]["start"]
        refs = language_server.request_references(file_path, sel_start["line"], sel_start["character"])
        assert any(
            "main.d" in (ref.get("relativePath") or "") for ref in refs
        ), "main.d should reference Helper (tried all positions in selectionRange)"
